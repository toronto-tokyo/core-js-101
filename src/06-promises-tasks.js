...
async function chainPromises(array, action) {
  ...
  await new Promise((resolve) => {
    array.forEach((promise) => {
      promise
        .then((promiseResult) => resultsArr.push(promiseResult))
        .catch();
    });
    resolve('isFinished');
  });
  let result = action(resultsArr[0], resultsArr[1]);

  for (let i = 2; i <= resultsArr.length - 1; i += 1) {
    result = action(result, resultsArr[i]);
  }
  return result;
}
